...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	// your code here
  	float blocks = grid.size() * grid[0].size();
  	float value = 1/blocks;
  ...
  float blurring) 
{
  ...
  for (int i = 0; i < beliefs.size(); i++) {
  	for (int j = 0; j < beliefs[0].size(); j++) {
    	newGrid[i][j] = beliefs[(i-dy+beliefs.size()) % beliefs.size()][(j-dx+beliefs[0].size()) % beliefs[0].size()];
    }
  }
	...
	float p_miss) 
{
  ...
  for (int i = 0; i < beliefs.size(); i++) {
  	for (int j = 0; j < beliefs[0].size(); j++) {
    	if (grid[i][j] == color) {
        	newGrid[i][j] = p_hit * beliefs[i][j];
        } else {
        	newGrid[i][j] = p_miss * beliefs[i][j];
        }
    }
  }
